...
const fourTestsPassedRegex = /4 tests passed in \d+(\.\d+)?ms/
const oneTestPassedRegex = /1 test passed in \d+(\.\d+)?ms/
...
test('sample.filter-test', async (): Promise<void> => {
  ...
  expect(stdout).toMatch(fourTestsPassedRegex)
})
...
test('sample.filter-test with filter=A', async (): Promise<void> => {
  ...
  expect(stdout).toMatch(oneTestPassedRegex)
})
